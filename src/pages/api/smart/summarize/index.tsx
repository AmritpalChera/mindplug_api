...
import summarizeText from '@/utils/chatbot/summarizer';
...
const bodySchema = object({
  text: string(),
  amountCharacters: number().optional()
})
...
export default async function handler(req: FetchRequest, res: NextApiResponse<Data>) {
  ...
  if (req.method === 'POST') {
    ...
    const { text, amountCharacters } = req.body;

    try {
      const content = await summarizeText({search: text, amountChars: amountCharacters || 300 });
